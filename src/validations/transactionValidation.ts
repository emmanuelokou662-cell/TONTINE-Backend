...
export const declarePaymentSchema = z.object({
  id_groupe: z.string().min(1, 'Identifiant du groupe invalide'),
  id_tour: z.string().min(1, 'Identifiant du tour invalide').optional(),
...
export const validatePaymentSchema = z.object({
  id_transaction: z.string().min(1, 'Identifiant de transaction invalide')
});
...
export const confirmDistributionSchema = z.object({
  id_tour: z.string().min(1, 'Identifiant du tour invalide'),
...
export const setTourOrderSchema = z.object({
  id_cycle: z.string().min(1, 'Identifiant de cycle invalide'),
  ordres: z.array(
    z.object({
      id_membre_groupe: z.string().min(1),
